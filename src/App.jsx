...
export default function App() {
  ...
  const uploadFile = async (file, folder) => {
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from("notes-images") // make sure this matches your exact bucket name
  ...
  // --- HANDLERS ---
  const handleImage = (e) => {
    if (e.target.files.length > 0) setTaskImage(e.target.files[0]);
  };

  const handleVideo = (e) => {
    if (e.target.files.length > 0) setTaskVideo(e.target.files[0]);
  };
